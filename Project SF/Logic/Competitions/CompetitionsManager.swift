...
class CompetitionsManager {
                        ...
                        operation.completionBlock = {
                            ...
                            self.inviteFriendsToCompetition(friends, inviteURL: url, zoneID: zone.zoneID) { result in
                                ...
                                case .success(let scoreURLHolderShareURLs):
                                    ...
                                    let finalSaveOperation = CKModifyRecordsOperation(recordsToSave: [
    ...
    func acceptInvitation(_ invitation: CompetitionInvitationRecord, then handler: @escaping (Result<Void, Error>) -> Void) {
        guard let competitionRecordInviteURLString = invitation.competitionRecordInviteURL,
            ...
            handler(.failure(CompetitionsManagerError.missingURL))
            return
        }
    ...
    func fetchScoreRecordsFor(
        _ competition: CompetitionRecord,
    ...
    private func fetchCompetitionsFromDatabaseWithScope(
        _ scope: CKDatabase.Scope,
        then handler: @escaping (Result<[CompetitionRecord], Error>) -> Void
    ) {
    ...
    private func fetchScoreRecordInfomation(
    ...
    private func fetchExternalScoreRecordsFor(
        _ competition: CompetitionRecord,
        then handler: @escaping (Result<[ScoreRecord], Error>) -> Void
    ) {
        // TODO: Add caching
    ...
    // MARK: Competition Manager Error
    
    enum CompetitionsManagerError: Error {
        case unknownError
